...
class GateService(object):
    ...
    def __init__(self):
        ...
        self.auto_close_timeout = config.getint('system', 'auto_close_timeout', 10)
    ...
    def open_gate(self, auto_close_timeout=None):
        ...
        try:
            ...
            if success:
                ...
                timeout = auto_close_timeout or self.auto_close_timeout
                if timeout > 0:
                    self._set_auto_close_timer(timeout)
    ...
    def _set_auto_close_timer(self, timeout):
        """Set auto-close timer"""
        self._clear_auto_close_timer()
        
        def auto_close():
            time.sleep(timeout)
            if self.current_status == GateStatus.OPEN:
                logger.info("Auto-closing gate after {} seconds".format(timeout))
                self.close_gate()
        
        self.auto_close_timer = threading.Thread(target=auto_close)
        self.auto_close_timer.daemon = True
        self.auto_close_timer.start()
    
    def _clear_auto_close_timer(self):
        """Clear auto-close timer"""
        if self.auto_close_timer and self.auto_close_timer.is_alive():
            # Note: In Python 2.7, we can't cleanly cancel threads
            # The timer will just check status before closing
            pass
        self.auto_close_timer = None

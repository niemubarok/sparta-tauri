...
def test_camera_service():
    ...
    try:
        ...
        # Test exit camera (primary)
        print("Testing exit camera...")
        result = camera_service.capture_image('exit')
        
        if result.success:
            print("✅ EXIT camera capture: SUCCESS")
            print("  Image size: {} bytes".format(len(result.image_data) if result.image_data else 0))
        else:
            print("❌ EXIT camera capture: FAILED")
            print("  Error: {}".format(result.error_message))
        
        # Test driver camera if available
        camera_status = camera_service.get_cameras_status()
        if 'driver' in camera_status:
            print("Testing driver camera...")
            result = camera_service.capture_image('driver')
            
            if result.success:
                print("✅ DRIVER camera capture: SUCCESS")
                print("  Image size: {} bytes".format(len(result.image_data) if result.image_data else 0))
            else:
                print("❌ DRIVER camera capture: FAILED")
                print("  Error: {}".format(result.error_message))
        else:
            print("ℹ️  Driver camera not configured (optional)")

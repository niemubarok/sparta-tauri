/**
 * Script untuk menginisialisasi database member dengan data sample
 * Jalankan dengan: node scripts/init-member-database.js
 */
...
const memberDb = new PouchDB('sparta_members')
const membershipTypeDb = new PouchDB('sparta_membership_types')
...
async function initializeDatabase() {
  try {
    ...
    for (const type of sampleMembershipTypes) {
      try {
        await membershipTypeDb.put(type)
    ...
    for (const index of memberIndexes) {
      try {
        await memberDb.createIndex({ index })
    ...
    for (const index of typeIndexes) {
      try {
        await membershipTypeDb.createIndex({ index })
    ...
    // Get statistics
    const memberResult = await memberDb.find({
      selector: { type: 'member' }
    })
    
    const typeResult = await membershipTypeDb.find({
...
initializeDatabase().then(() => {
  console.log('✨ All done! You can now use the member management system.')
  process.exit(0)
}).catch(error => {
